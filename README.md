...
- **OBS オーバーレイ** — 内蔵HTTP/WebSocketサーバでコメントをリアルタイム配信。テンプレート(default/simple)・フォント倍率・最大行数・表示時間・背景透過・表示位置などを設定可能
...
- **設定永続化** (`src-tauri/src/config.rs`): `config.json` を唯一の永続化先とする（localStorage 非使用）。新フィールドは `serde(default)` で後方互換
...
`build-windows.bat` は setup を統合した単一エントリです。Rust / MSVC が無ければ自動インストールし、そのままビルドまで進みます。生成されたインストーラー(`*-setup.exe`)は自動起動します。
